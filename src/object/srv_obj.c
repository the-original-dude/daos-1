	     ...
	     uint32_t *map_ver)
{
	...
	*hdlp = NULL;
	*contp = NULL;
	rc = ds_check_container(pool_uuid, hdl_uuid, co_uuid, opc, hdlp, contp);
	if (rc)
		return rc;
...
ds_obj_rw_handler(crt_rpc_t *rpc)
{
	...
	if (opc == DAOS_OBJ_RPC_FETCH) {
		...
		if (rc != 0) {
			D_ERROR(DF_UOID": rw_local (fetch) failed %d.\n",
				DP_UOID(orw->orw_oid), rc);
			D_GOTO(out, rc);
		}
 ...
 */
int
daos_enum_pack(vos_iter_param_t *param, vos_iter_type_t type, bool recursive,
	    ...
	    struct daos_enum_arg *enum_arg, uint32_t *map_version)
{
	...
	rc = daos_enum_pack(&param, type, recursive, anchors, enum_arg);
